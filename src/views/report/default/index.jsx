...
const ReportIndex = () => {
    ...
    const deleteItem = async () => {
        ...
        setIsLoading(true);
        let results = await fetch(`${process.env.REACT_APP_API_URL}/api/admin-user/deleteReportById`, {
            method: 'POST',
            headers: {
    ...
    const getAllRecords = async (status) => {
        const token = localStorage.getItem('LuminixLoginToken');
        fetch(`${process.env.REACT_APP_API_URL}/api/admin-user/getAllReport`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
            ...
            .then((result) => {
                // console.log(result)
                setReportTableList(result.report);
    ...
    const getReportStatusData = (data) => {
        setStatus(data)
        getAllRecords(data)
    }
        ...
        <>
            <div className="p-2 my-3 flex bg-white dark:!bg-navy-700">
                <div className="w-1/4">
                    ...
                    <input type="text" id="search-input" placeholder="Search by names" onChange={(e) => setSearch(e.target.value)} className="input  w-full max-w-xs text-neutral-900 dark:text-white bg-gray-200 dark:!bg-navy-800 placeholder:text-neutral-800 placeholder:dark:text-white" />
                ...
                <table className="table table-md">
                    <thead className="sticky top-0 bg-white dark:!bg-navy-800 text-neutral-900 dark:text-white">
                        <tr className="text-md">
                            ...
                            <th>Created date</th>
                            <th>
                                <select name="jobStatus" id="jobStatus" className="h-12 items-center justify-center rounded-xl bg-white/0 p-3 text-sm outline-none" onChange={e => getReportStatusData(e.target.value)}>
                    ...
                    <tbody>
                        {reportTableList.map((ele, index) => (
                            <tr key={ele._id} className="border-b border-gray-400 text-neutral-900 bg-white text-black dark:bg-navy-800 dark:text-white">
                                ...
                                <td>{ele.reportingPerson?.firstName} {ele.reportingPerson?.lastName}</td>
                                <td>{ele.reportedPerson?.firstName} {ele.reportedPerson?.lastName}</td>
                                <td className="text-center">{ele.reviewId}</td>
                                <td>{ele.createdAt && (new Date(ele.createdAt)).toLocaleString('en-US', {
                                    day: 'numeric',
                                    month: 'short',
                                    year: 'numeric',
                                })}</td>
                                <td className="text-center">{ele.status === "pending" ? "No action taken" : ele.status === "review" ? "Being reviewed" : "Resolved"}</td>
                                <td className="flex justify-around">
                                    <Link to={`/report/edit?id=${ele._id}`}><MdOutlineEdit className="ml-1 h-4 w-4" /></Link>

                                    {/* <MdDeleteOutline className="ml-1 h-4 w-4" onClick={() => showDeleteBox(ele._id)} /> */}
                                </td>
                            </tr>
                        ))}
                    </tbody>
