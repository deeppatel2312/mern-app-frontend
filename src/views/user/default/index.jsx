...
const UserIndex = () => {
    ...
    const loadUsers = async () => {
        ...
        setIsLoading(true);
        let results = await fetch(`${process.env.REACT_APP_API_URL}/api/admin-user/findAll`, {
            method: 'POST',
            headers: {
    ...
    const deleteItem = async () => {
        ...
        const token = localStorage.getItem('LuminixLoginToken');
        setIsLoading(true);
        let results = await fetch(`${process.env.REACT_APP_API_URL}/api/admin-user/deleteById`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
                ...
                <table className="table table-md">
                    ...
                    <tbody>
                        {users.map((user, index) => (
                            <tr key={user._id} className="border-b border-gray-400 text-neutral-900 bg-white text-black dark:bg-navy-800 dark:text-white">
                                ...
                                <td className="flex justify-around">
                                    <Link to={`/user/edit?id=${user._id}`}><MdOutlineEdit className="ml-1 h-4 w-4" /></Link>
                                    {/* <Link to={`/user/delete?id=${user._id}`}><MdDeleteOutline className="ml-1 h-4 w-4" /></Link> */}
